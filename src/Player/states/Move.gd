...
func physics_process(delta: float) -> void:
	...
	Events.emit_signal("player_moved", velocity)


static func calculate_velocity(old_velocity: Vector2, max_speed: Vector2, 
		acceleration: Vector2, decceleration: Vector2, delta: float, 
		move_direction: Vector2, max_speed_fall := 1500.0
	) -> Vector2:
